# IDA Python script for ESP32-C3 (RISC-V) firmware: ida_analyze_app1_riscv.py
# Usage (headless):
# ida64.exe -A -S"ida_analyze_app1_riscv.py" "d:\\esp32c3x4\\app1.bin"
# Note: For raw binary `app1.bin` (ESP32 partition image) prefer to load it in IDA
...
# Dump segments
with open(os.path.join(out_dir, "segments_app1.txt"), "w", encoding="utf-8") as segf:
...
with open(os.path.join(out_dir, "functions_app1.txt"), "w", encoding="utf-8") as ff:
...
# Dump strings
try:
    sfile = open(os.path.join(out_dir, "strings_app1.txt"), "w", encoding="utf-8")
...
with open(os.path.join(out_dir, "full_disasm_app1.asm"), "w", encoding="utf-8") as d:
...
with open(os.path.join(out_dir, "xrefs_app1.txt"), "w", encoding="utf-8") as xf:
    for func_ea in idautils.Functions():
        for ref in idautils.CodeRefsTo(func_ea, 0):
            xf.write("0x%08X -> 0x%08X\n" % (ref, func_ea))

# Export symbols (names and addresses)
with open(os.path.join(out_dir, "symbols_app1.txt"), "w", encoding="utf-8") as sf:
    for ea, name in idautils.Names():
        sf.write("0x%08X\t%s\n" % (ea, name))

# Export data segments (non-code)
with open(os.path.join(out_dir, "data_segments_app1.txt"), "w", encoding="utf-8") as df:
    for seg_ea in idautils.Segments():
...
with open(os.path.join(out_dir, "headers_app1.txt"), "w", encoding="utf-8") as hf:
    hf.write("File: %s\n" % input_file)
...
print("RISC-V analysis finished for app1.bin. Outputs: segments_app1.txt, functions_app1.txt, strings_app1.txt (if available), full_disasm_app1.asm, xrefs_app1.txt, symbols_app1.txt, data_segments_app1.txt, headers_app1.txt")
