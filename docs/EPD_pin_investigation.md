...
### 功能描述
已为项目添加按键触发深度睡眠功能，实现"按一下键就休眠"的电源管理需求。同时支持双击按键触发EPD重新测试功能。

### 硬件要求
- **按键连接**：按键连接到 GPIO 9 (SLEEP_BUTTON_PIN)
- **按键类型**：常开按键，连接 GPIO 9 到 GND (使用内部上拉电阻)
- **唤醒方式**：按RESET按键唤醒（ESP32-C3深度睡眠后）

### 软件实现
- **深度睡眠支持**：利用 ESP32-C3 的深度睡眠功能
- **按键检测**：轮询检测 GPIO 9 的电平变化，支持单击和双击识别
- **双击检测**：500ms时间窗口内的两次按键识别为双击
- **防抖处理**：50ms防抖延迟，避免误触发

### 使用方法
1. 程序启动后会显示：
   - "Press button on GPIO 9 to enter deep sleep..."
   - "Double-click button on GPIO 9 to re-test EPD display..."
2. **单击按键**：按键按下后1秒进入深度睡眠模式
3. **双击按键**：在500ms内按两次键，重新测试EPD显示功能
4. 按RESET按键唤醒系统
...
| 操作 | 功能 | 说明 |
|------|------|------|
| **单击** | 深度睡眠 | 按键按下后1秒进入深度睡眠 |
| **双击** | EPD重测 | 在500ms内按两次键，重新测试EPD显示 |
...
- 双击时间窗口：`DOUBLE_CLICK_TIME = 500ms`

此功能为系统提供了完整的电源管理解决方案，结合 EPD 的软件休眠和 ESP32 的硬件深度睡眠，实现超低功耗待机模式。同时提供便捷的EPD功能验证机制。
