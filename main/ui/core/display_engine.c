...
static sFONT* choose_ascii_font_by_cjk_height_menu(void)
{
    ...
    if (cjk_h <= 0) {
        ESP_LOGW(TAG, "Menu font height is 0, using Font16 as fallback");
        return &Font16;
    }
    return choose_ascii_font_by_target_height(cjk_h);
}
...
static int draw_text_utf8_menu_locked(int x, int y, const char *text, sFONT *ascii_font, uint8_t color, uint8_t bg_color)
{
    ...
    // 检查菜单字体是否可用
    if (xt_eink_font_get_menu_default_font() == NULL) {
        ESP_LOGW(TAG, "Menu font unavailable, rendering text as ASCII only: %s", text);
        // 降级：仅渲染 ASCII 字符
        int cur_x = x;
        for (const char *p = text; *p != '\0'; p++) {
            if ((unsigned char)*p <= 0x7F) {
                Paint_DrawChar(cur_x, y, *p, ascii_font, color, bg_color);
                cur_x += (int)ascii_font->Width;
            } else {
                // 跳过非 ASCII 字符
                Paint_DrawChar(cur_x, y, '?', ascii_font, color, bg_color);
                cur_x += (int)ascii_font->Width;
            }
        }
        return cur_x - x;
...
void display_refresh(refresh_mode_t mode)
{
    ...
    ESP_LOGI(TAG, "Before refresh: DMA free=%zu, largest=%zu", free_heap, largest_block);
    
    // SPI DMA 需要至少几KB的连续内存，如果内存不足则等待或跳过
    if (largest_block < 4096) {
        ESP_LOGW(TAG, "Low DMA memory! Skipping refresh to prevent crash");
        // 尝试触发垃圾回收
        vTaskDelay(pdMS_TO_TICKS(100));
        largest_block = heap_caps_get_largest_free_block(MALLOC_CAP_DMA);
        if (largest_block < 4096) {
            ESP_LOGE(TAG, "Still low DMA memory (%zu), aborting refresh", largest_block);
            return;
        }
        ESP_LOGI(TAG, "DMA memory recovered to %zu", largest_block);
    }
...
{
    ESP_LOGI(TAG, "display_draw_text_menu ENTER: text='%s' x=%d y=%d", text ? text : "(null)", x, y);
    
    if (text == NULL || *text == '\0') {
        ESP_LOGW(TAG, "display_draw_text_menu: text is NULL or empty");
        return 0;
    }

    ESP_LOGI(TAG, "Calling display_get_menu_font...");
    sFONT *font = display_get_menu_font();
    ESP_LOGI(TAG, "display_get_menu_font returned: %p", (void*)font);
    ...
    ESP_LOGI(TAG, "Font check: Width=%d Height=%d table=%p", 
             font->Width, font->Height, (void*)font->table);
    ...
    // 检查 Paint.Image
    ESP_LOGI(TAG, "Paint.Image=%p s_framebuffer=%p", (void*)Paint.Image, (void*)s_framebuffer);
    if (Paint.Image == NULL) {
    ...
    ESP_LOGI(TAG, "Calling lock_engine...");
    lock_engine();
    ESP_LOGI(TAG, "lock_engine done, starting draw loop");
    
    int width;
    if (text_has_non_ascii(text)) {
        ESP_LOGI(TAG, "Drawing UTF-8 text");
        width = draw_text_utf8_menu_locked(x, y, text, font, color, bg_color);
    } else {
        ESP_LOGI(TAG, "Drawing ASCII text");
        const int ascii_adv = (int)font->Width;
        int cur_x = x;
        for (const char *p = text; *p != '\0'; p++) {
            ESP_LOGD(TAG, "Drawing char '%c' at x=%d", *p, cur_x);
    ...
    ESP_LOGI(TAG, "display_draw_text_menu EXIT: width=%d", width);
