...
static void chapter_loading_timer_cb(TimerHandle_t xTimer)
{
    ...
    if (x4im_rx_mutex != NULL && xSemaphoreTake(x4im_rx_mutex, 0) == pdTRUE) {
        if (x4im_rx_state.receiving && !x4im_rx_state.use_sd_card && x4im_rx_state.expected_size > 0) {
...
static void start_chapter_loading_progress(const char *prompt_text)
{
    ...
    if (prompt_text && *prompt_text) {
        strncpy(s_ble_state.chapter_loading_prompt, prompt_text, sizeof(s_ble_state.chapter_loading_prompt) - 1);
        s_ble_state.chapter_loading_prompt[sizeof(s_ble_state.chapter_loading_prompt) - 1] = '\0';
    } else {
        s_ble_state.chapter_loading_prompt[0] = '\0';
    }

    if (s_chapter_loading_timer == NULL) {
        s_chapter_loading_timer = xTimerCreate("ch_load", pdMS_TO_TICKS(3000), pdTRUE, NULL, chapter_loading_timer_cb);
        if (s_chapter_loading_timer == NULL) {
            ...
            s_ble_state.chapter_loading_last_shown = s_ble_state.chapter_loading_fake_percent;
            return;
        }
    } else {
...
static void ble_data_received_callback(const uint8_t *data, uint16_t length)
{
    ...
    if ((length == 1 && data[0] == X4IM_CMD_SHOW_PAGE) || 
        ...
        // 优先尝试 JPG 格式
        snprintf(image_path, sizeof(image_path), "/littlefs/ble_vfs/page_%u.jpg", page_index);
        if (stat(image_path, &st) == 0) {
            image_found = true;
            ESP_LOGI(TAG, "Found JPG image: %s (%ld bytes)", image_path, (long)st.st_size);
        } else {
            // 尝试 BMP 格式
            snprintf(image_path, sizeof(image_path), "/littlefs/ble_vfs/page_%u.bmp", page_index);
            if (stat(image_path, &st) == 0) {
                image_found = true;
                ESP_LOGI(TAG, "Found BMP image: %s (%ld bytes)", image_path, (long)st.st_size);
            }
        }
        
        if (!image_found) {
            ESP_LOGW(TAG, "Image file not found (JPG or BMP) for page_%u", page_index);
            free((void *)data);
            return;
        }
        ...
        if (success) {
            ...
            // 显示错误信息
            display_draw_text_menu(40, SCREEN_HEIGHT/2, "Image render failed", COLOR_BLACK, COLOR_WHITE);
            display_refresh(REFRESH_MODE_FULL);
        }
            ...
            if (fp != NULL) {
                ...
                if (written == payload_length) {
                    if (g_ble_new_transfer) {
                        ...
                        snprintf(s_ble_state.bmp_receiving_path, sizeof(s_ble_state.bmp_receiving_path), "%s", bmp_path);
        ...
        if (x4im_flags & X4IM_FLAGS_TYPE_JPG) {
            ...
            if (x4im_filename[0] != '\0') {
                ...
                if (x4im_filename[0] == '/') {
                    ...
                    snprintf(jpg_path, sizeof(jpg_path), "/littlefs/ble_vfs/%s", x4im_filename);
                }
            } else {
            ...
            if (fp != NULL) {
                ...
                if (written == payload_length) {
                    if (g_ble_new_transfer) {
                        ESP_LOGI(TAG, "JPG: New file created, wrote %zu bytes to %s", written, jpg_path);
                        g_ble_new_transfer = false;
                        // 记录正在接收JPG
                        s_ble_state.receiving_jpg = true;
                        snprintf(s_ble_state.jpg_receiving_path, sizeof(s_ble_state.jpg_receiving_path), "%s", jpg_path);
                        s_ble_state.transfer_bytes_received = written;
                        s_ble_state.transfer_bytes_total = declared_payload_size;
                    } else {
    ...
    if (s_ble_state.receiving_bmp && !has_x4im_header && payload_length > 0) {
        ...
        if (fp != NULL) {
            ...
            if (written == payload_length) {
                s_ble_state.transfer_bytes_received += written;
                ESP_LOGD(TAG, "BMP: Appended %zu bytes, total: %lu/%lu", 
            ...
            ESP_LOGE(TAG, "BMP: Failed to open file for append: %s", s_ble_state.bmp_receiving_path);
        }
    ...
    if (s_ble_state.receiving_jpg && !has_x4im_header && payload_length > 0) {
        ...
        if (fp != NULL) {
            ...
            if (written == payload_length) {
                s_ble_state.transfer_bytes_received += written;
                ESP_LOGD(TAG, "JPG: Appended %zu bytes, total: %lu/%lu", 
                         written, (unsigned long)s_ble_state.transfer_bytes_received,
                         (unsigned long)s_ble_state.transfer_bytes_total);
            ...
            ESP_LOGE(TAG, "JPG: Failed to open file for append: %s", s_ble_state.jpg_receiving_path);
        }
...
static void __attribute__((unused)) send_chapter_navigate_request(uint8_t direction)
{
    ...
    // 显示加载提示并启动进度更新
    const char *prompt = direction == CHAPTER_NAV_NEXT ? "正在加载下一章..." : "正在加载上一章...";
