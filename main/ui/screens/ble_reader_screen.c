...
#include "nvs.h"
#include "nvs_flash.h"
#include "wallpaper_manager.h" // 统一的图片渲染API
...
typedef struct {
  ...
  // ===== 翻页游标与历史 =====
  size_t
      last_page_consumed; // 最近一次绘制实际消耗的显示字符数（不含换行，忽略换行后的前导空白）
  ...
  bool receiving_bmp;                   // 是否正在接收BMP文件
...
static ble_reader_state_internal_t s_ble_state = {
    ...
    .receiving_bmp = false,
    .bmp_receiving_path = {0},
    .receiving_jpg = false,
    .jpg_receiving_path = {0},
...
// 协议回调
static bool __attribute__((unused)) on_page_ready(uint16_t book_id,
                                                  uint16_t page_num);
static void __attribute__((unused))
on_preload_needed(uint16_t book_id, uint16_t start_page, uint8_t page_count);
...
static void __attribute__((unused)) handle_reading_mode_button(screen_t *screen,
                                                               button_t btn);

// 翻页防抖和同步
static void __attribute__((unused))
send_page_sync_notification(uint16_t page_num);
...
static esp_err_t ble_nvs_save_state(void) {
  ...
  // 准备保存的数据
  ble_nvs_state_t state = {.book_hash = s_ble_state.current_book_hash,
  ...
  // 复制书名
  strncpy(state.book_title, s_ble_state.book_title,
          sizeof(state.book_title) - 1);
  state.book_title[sizeof(state.book_title) - 1] = '\0';

  // 保存到 NVS
  err =
      nvs_set_blob(handle, BLE_NVS_KEY_STATE, &state, sizeof(ble_nvs_state_t));
  if (err != ESP_OK) {
  ...
  if (err != ESP_OK) {
    ...
    ESP_LOGI(TAG, "BLE reading state saved: book=%08x chapter=%08x pos=%zu/%zu",
             state.book_hash, state.chapter_hash, state.char_position,
             state.total_chars);
  }
...
static esp_err_t ble_nvs_load_state(void) {
  ...
  // 复制书名
  strncpy(s_ble_state.book_title, state.book_title,
          sizeof(s_ble_state.book_title) - 1);
  ...
  ESP_LOGI(
      TAG,
      "BLE reading state loaded: book=%s (hash=%08x) chapter=%08x pos=%zu/%zu",
      s_ble_state.book_title, s_ble_state.current_book_hash,
      s_ble_state.current_chapter_hash, s_ble_state.char_position,
      s_ble_state.total_chars);
...
static size_t process_wrapped_text(int x, int y, int max_width, int max_height,
  ...
  while (*p && lines_filled < lines_can_show) {
    ...
    if (line_len + clen >= (int)sizeof(line) - 1) {
      ...
      cur_y += line_height;
      lines_filled++;
      if (lines_filled >= lines_can_show)
        break;
      // 不移动 p，下次循环重新处理这个字符
      continue;
    }
    ...
    // 检测超宽：当前字符导致行超出宽度限制
    if (display_get_text_width_font(line, font) > max_width &&
        line_len > clen) {
      // 回退当前字符，输出当前行
      line_len -= clen;
      line[line_len] = '\0';
...
static void __attribute__((unused))
draw_wrapped_text(int x, int y, int max_width, int max_height, const char *text,
                  size_t char_offset) {
  process_wrapped_text(x, y, max_width, max_height, text, char_offset, true);
}

static size_t __attribute__((unused))
measure_wrapped_consumed(int max_width, int max_height, const char *text,
                         size_t char_offset) {
  return process_wrapped_text(0, 0, max_width, max_height, text, char_offset,
                              false);
}
...
#define X4IM_FLAGS_TYPE_JPG 0x0040   // Bit 6: JPG 图片类型
 ...
 */
static void __attribute__((unused))
scan_directory_recursive(const char *path, int *file_count, int *dir_count,
                         uint64_t *used_size) {
  // 限制递归深度，避免栈溢出（最大10层）
  static int depth = 0;
  if (depth > 10) {
...
static void ble_data_received_callback(const uint8_t *data, uint16_t length) {
  ...
  if (current_screen == NULL || current_screen != &g_ble_reader_screen) {
    ESP_LOGW(TAG,
             "BLE data received but screen is not active, discarding %u bytes",
             length);
    free((void *)data);
    return;
  }
  ...
  if ((length == 1 && data[0] == X4IM_CMD_SHOW_PAGE) ||
    ...
    // 尝试加载并显示图片（支持 BMP 和 JPG）
    char image_path[128];
    struct stat st;
    bool image_found = false;

    // 优先尝试 JPG 格式
    snprintf(image_path, sizeof(image_path), "/littlefs/ble_vfs/page_%u.jpg",
             page_index);
    if (stat(image_path, &st) == 0) {
      image_found = true;
      ESP_LOGI(TAG, "Found JPG image: %s (%ld bytes)", image_path,
               (long)st.st_size);
    } else {
      // 尝试 BMP 格式
      snprintf(image_path, sizeof(image_path), "/littlefs/ble_vfs/page_%u.bmp",
               page_index);
      if (stat(image_path, &st) == 0) {
        image_found = true;
        ESP_LOGI(TAG, "Found BMP image: %s (%ld bytes)", image_path,
                 (long)st.st_size);
      }
    }

    if (!image_found) {
      ESP_LOGW(TAG, "Image file not found (JPG or BMP) for page_%u",
               page_index);
      free((void *)data);
      return;
    }

    // 使用统一的图片渲染API（wallpaper_manager）
    // 先清空屏幕
    display_clear(COLOR_WHITE);

    // 使用 wallpaper_render_image_to_display 显示图片（支持JPG/BMP/PNG）
    bool success = wallpaper_render_image_to_display(image_path);
    if (success) {
      ESP_LOGI(TAG, "Image displayed successfully via wallpaper_manager");
      // 刷新屏幕显示
      display_refresh(REFRESH_MODE_FULL);
      ESP_LOGI(TAG, "Display refreshed");
    } else {
      ESP_LOGE(TAG, "Failed to render image via wallpaper_manager");
      // 显示错误信息
      display_draw_text_menu(40, SCREEN_HEIGHT / 2, "Image render failed",
  ...
  if (is_command) {
    ...
    if (cmd == X4IM_CMD_CHAPTER_INFO) {
      ...
      if (length >= 6) {
        ...
        ESP_LOGI(TAG,
                 "Received CHAPTER_INFO header: total=%u, json_len=%zu, "
                 "first_chunk=%zu",
                 total_ch, json_len, header_json_bytes);
        ...
        if (json_len == 0) {
          ...
          if (!s_chinfo_rx.buf) {
            ESP_LOGE(TAG, "Failed to alloc chapter JSON buffer (%zu)",
                     json_len);
            free((void *)data);
            return;
          }
  ...
  if (s_chinfo_rx.receiving) {
    if (length > 0 && s_chinfo_rx.buf &&
        s_chinfo_rx.received < s_chinfo_rx.expected) {
      ...
      ESP_LOGI(TAG, "Accumulating CHAPTER_INFO JSON: %zu/%zu",
               s_chinfo_rx.received, s_chinfo_rx.expected);
      if (s_chinfo_rx.received >= s_chinfo_rx.expected) {
        s_chinfo_rx.buf[s_chinfo_rx.expected] = '\0';
        parse_chapter_info_json(s_chinfo_rx.buf, s_chinfo_rx.expected);
  ...
  // ========== 检测并解析 X4IM v2 协议头 ==========
  // X4IM v2 头部: "X4IM" (4B) + version(2B) + flags(2B) + payload_size(4B) +
  // sd(4B) + name(16B)
  ...
  if (length >= X4IM_HEADER_SIZE && data[0] == 'X' && data[1] == '4' &&
    ...
    ESP_LOGI(
        TAG,
        "X4IM v2 header: type=0x%02X, flags=0x%04X, payload=%lu, name='%s'",
  ...
  if (payload_length >= 4) {
    if ((payload_length == 5 && payload_data[0] == 0x00 &&
         payload_data[1] == 'E' && payload_data[2] == 'O' &&
         payload_data[3] == 'F' && payload_data[4] == '\n') ||
        (payload_length == 4 && payload_data[0] == 0x00 &&
         payload_data[1] == 'E' && payload_data[2] == 'O' &&
         payload_data[3] == 'F')) {
      is_eof_marker = true;
  ...
  if (s_ble_state.vfs_book != NULL && payload_length > 0) {
    ...
    snprintf(chapter_path, sizeof(chapter_path),
             "/littlefs/ble_vfs/current_ch%d.txt", current_chapter);
    ...
    if (is_eof_marker) {
      ESP_LOGI(TAG,
               "=== Transfer complete for chapter %d ===", current_chapter);
      ...
      if (stat(chapter_path, &file_st) == 0) {
        ...
        ESP_LOGI(TAG, "File ready: %s (%zu bytes)", chapter_path,
                 s_ble_state.total_chars);
      } else {
    ...
    if (fp != NULL) {
      ...
      ESP_LOGE(TAG, "Failed to open chapter file for writing: %s (mode=%s)",
               chapter_path, mode);
    }
  } else if (payload_length == 0 && !is_eof_marker) {
    ESP_LOGW(TAG, "Empty payload received (length=%u, has_x4im=%d)", length,
             has_x4im_header);
  }
 ...
 * @param length 数据长度
 */
static void __attribute__((unused)) handle_page_data(const uint8_t *data,
                                                     size_t length) {
  // 检查是否是 X4IM v2 帧头（32 字节）
  // "X4IM" + version(2) + flags(2) + payload_size(4) + sequence(2) +
  // reserved(2) + filename(16)
  if (length >= X4IM_HEADER_SIZE && data[0] == 'X' && data[1] == '4' &&
      data[2] == 'I' && data[3] == 'M' && data[4] == 0x02) {
    ...
    ESP_LOGI(TAG,
             "X4IM frame: v2, flags=0x%04X, payload=%" PRIu32
             ", sd=%d, name='%s'",
    ...
    if (xSemaphoreTake(x4im_rx_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
      ...
      if (use_sd) {
        ...
        if (recv_filename[0] != '\0') {
          ...
          snprintf(x4im_rx_state.filename, sizeof(x4im_rx_state.filename),
                   ...
                   s_ble_state.current_book_id ? s_ble_state.current_book_id
                                               : 1,
      ...
      if (x4im_rx_state.file_handle == NULL) {
        ...
        ESP_LOGE(TAG, "Failed to open file for streaming: %s (errno=%d)",
                 x4im_rx_state.filename, errno);
        free((void *)data); // 释放内存
        return;
      }

      ESP_LOGI(TAG, "Opened file for streaming: %s (SD=%d)",
               x4im_rx_state.filename, use_sd);
      ...
      if (s_ble_state.work_mode == BLE_MODE_TRANSFER) {
        ...
        if (log_file != NULL) {
          fprintf(log_file, "File: %s | Path: %s | Size: %" PRIu32 " bytes\n",
                  recv_filename[0] ? recv_filename : "unknown",
                  x4im_rx_state.filename, payload_size);
          fclose(log_file);
        }
      }
      ...
      if (length > header_size) {
        ...
        size_t written =
            fwrite(data + header_size, 1, copy_len, x4im_rx_state.file_handle);
        if (written != copy_len) {
          ESP_LOGE(TAG, "File write error: expected %" PRIu32 ", wrote %lu",
                   copy_len, (unsigned long)written);
          fclose(x4im_rx_state.file_handle);
          x4im_rx_state.file_handle = NULL;
          x4im_rx_state.receiving = false;
          xSemaphoreGive(x4im_rx_mutex);
        ...
        ESP_LOGI(TAG,
                 "Wrote %" PRIu32 " bytes from header packet (%" PRIu32
                 "/%" PRIu32 ")",
                 copy_len, x4im_rx_state.received_size,
                 x4im_rx_state.expected_size);
      }
      ...
      if (complete) {
        ...
        ESP_LOGI(TAG, "Storage: %s",
                 x4im_rx_state.use_sd_card ? "SD Card" : "LittleFS");
        ESP_LOGI(TAG, "============================");
        ...
        if (s_ble_state.work_mode == BLE_MODE_TRANSFER) {
          ...
          ESP_LOGI(TAG, "Transfer mode: file #%u saved to SD card",
                   s_ble_state.transfer_file_count);
        } else {
          // 阅读模式：保持原有逻辑
          // 收到第一帧数据时初始化 book_id
          if (!s_ble_state.initialization_complete) {
            if (s_ble_state.current_book_id == 0) {
              ...
              ESP_LOGI(TAG, "First page received, book_id set to %04x",
                       s_ble_state.current_book_id);
            }
            s_ble_state.showing_confirm_prompt = true;
            s_ble_state.current_page = 0;
            ESP_LOGI(TAG,
                     "Showing confirm prompt: Click CONFIRM to start reading");
          }
  ...
  if (xSemaphoreTake(x4im_rx_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
    if (x4im_rx_state.receiving && x4im_rx_state.file_handle != NULL) {
      uint32_t remaining =
          x4im_rx_state.expected_size - x4im_rx_state.received_size;
      ...
      bool should_log =
          (x4im_rx_state.received_size - last_log_bytes >= 2440) ||
          (x4im_rx_state.received_size >= x4im_rx_state.expected_size);
      ...
      if (s_ble_state.work_mode == BLE_MODE_TRANSFER &&
          x4im_rx_state.expected_size > 0) {
        uint32_t progress_diff =
            x4im_rx_state.received_size - last_ui_update_bytes;
        uint32_t update_threshold = x4im_rx_state.expected_size / 20; // 5%
        should_update_ui =
            (progress_diff >= update_threshold) ||
            (x4im_rx_state.received_size >= x4im_rx_state.expected_size);
      }

      if (should_log) {
        ESP_LOGI(TAG,
                 "Streaming to file: %" PRIu32 "/%" PRIu32
                 " bytes (%.1f%%) | UI: %" PRIu32 "/%" PRIu32,
                 x4im_rx_state.received_size, x4im_rx_state.expected_size,
                 (float)x4im_rx_state.received_size * 100.0f /
                     x4im_rx_state.expected_size,
                 s_ble_state.transfer_bytes_received,
                 s_ble_state.transfer_bytes_total);
      ...
      if (complete) {
        ...
        if (s_ble_state.work_mode == BLE_MODE_TRANSFER) {
          ...
          ESP_LOGI(TAG, "Transfer mode: file #%u saved",
                   s_ble_state.transfer_file_count);
        } else {
          // 阅读模式：保持原有逻辑
          // 收到第一帧数据时初始化 book_id
          ...
          if (x4im_rx_state.logical_index ==
            ...
            if (stat(filename, &st) == 0) {
              s_ble_state.total_chars =
                  (size_t)st.st_size; // 存储文件大小（字节数）
              ESP_LOGI(TAG, "Page loaded: file size = %zu bytes",
                       s_ble_state.total_chars);
            } else {
              s_ble_state.total_chars = 0;
...
static int calculate_chars_per_screen(void) {
  ...
  const int max_height =
      SCREEN_HEIGHT - BLE_CONTENT_Y_START - BLE_CONTENT_BOTTOM_MARGIN;
...
static void draw_transfer_mode_screen(void) {
  ...
  char buf[96];
  snprintf(buf, sizeof(buf), "文件: %s",
           s_ble_state.transfer_filename[0] ? s_ble_state.transfer_filename
                                            : "(未知)");
  display_draw_text_menu(10, 40, buf, COLOR_BLACK, COLOR_WHITE);

  snprintf(buf, sizeof(buf), "进度: %lu / %lu bytes",
           (unsigned long)s_ble_state.transfer_bytes_received,
           (unsigned long)s_ble_state.transfer_bytes_total);
  display_draw_text_menu(10, 70, buf, COLOR_BLACK, COLOR_WHITE);

  snprintf(buf, sizeof(buf), "已完成: %u 个文件",
           s_ble_state.transfer_file_count);
...
static void draw_reading_mode_screen(bool clear_content) {
  ...
  snprintf(status_line + strlen(status_line),
           sizeof(status_line) - strlen(status_line), " %s", status_str);

  if (s_ble_state.current_book_id != 0) {
    if (s_ble_state.total_pages > 0) {
      snprintf(status_line + strlen(status_line),
               sizeof(status_line) - strlen(status_line), " %u/%u",
               s_ble_state.current_page + 1, s_ble_state.total_pages);
    } else {
      snprintf(status_line + strlen(status_line),
               sizeof(status_line) - strlen(status_line), " %u",
               s_ble_state.current_page + 1);
    }
  }

  // 先清除状态栏区域，避免状态切换时旧文字残留
  display_clear_region(0, top_y, SCREEN_WIDTH, BLE_CONTENT_Y_START - top_y,
                       COLOR_WHITE);
  display_draw_text_menu(0, top_y, status_line, COLOR_BLACK, COLOR_WHITE);

  // 显示初始化确认提示
  if (s_ble_state.current_book_id != 0 &&
      !s_ble_state.initialization_complete) {
    int center_y = SCREEN_HEIGHT / 2;
    display_draw_text_menu(20, center_y - 40, "点击确认开始阅读", COLOR_BLACK,
                           COLOR_WHITE);
    display_draw_text_menu(20, center_y, "按 确认 键", COLOR_BLACK,
                           COLOR_WHITE);
    s_ble_state.showing_confirm_prompt = true;
    display_draw_text_menu(20, SCREEN_HEIGHT - 40, "确认: 开始", COLOR_BLACK,
                           COLOR_WHITE);
  } else if (s_ble_state.current_book_id == 0) {
    // 检查是否有保存的阅读状态
    if (s_ble_state.char_position > 0 || s_ble_state.total_chars > 0) {
      // 有保存的阅读位置
      display_draw_text_menu(20, 100, "已保存的阅读内容：", COLOR_BLACK,
                             COLOR_WHITE);
      if (s_ble_state.book_title[0] != '\0') {
        display_draw_text_menu(20, 130, s_ble_state.book_title, COLOR_BLACK,
                               COLOR_WHITE);
      }
      display_draw_text_menu(20, 160, "按确认键继续读书，或", COLOR_BLACK,
                             COLOR_WHITE);
      display_draw_text_menu(20, 190, "等待手机发送新内容...", COLOR_BLACK,
                             COLOR_WHITE);
      s_ble_state.pending_local_resume = true;
    } else {
      // 无保存的内容
      display_draw_text_menu(20, 100, "未选择书籍", COLOR_BLACK, COLOR_WHITE);
      display_draw_text_menu(20, 140, "等待手机发送内容...", COLOR_BLACK,
                             COLOR_WHITE);
    ...
    if (clear_content) {
      ...
      int content_height =
          SCREEN_HEIGHT - BLE_CONTENT_Y_START - BLE_CONTENT_BOTTOM_MARGIN;
      display_clear_region(BLE_CONTENT_X_MARGIN, BLE_CONTENT_Y_START,
                           content_width, content_height, COLOR_WHITE);
    }
    ...
    if (!page_buffer) {
      ...
      // 使用互斥锁防止与 BLE 接收冲突
      if (x4im_rx_mutex != NULL &&
          xSemaphoreTake(x4im_rx_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
        vfs_seek(s_ble_state.vfs_book, s_ble_state.char_position, SEEK_SET);
        bytes_read =
            vfs_read(s_ble_state.vfs_book, page_buffer, BLE_TEXT_MAX_BYTES - 1);
        ...
        if (vfs_page_ready) {
          ...
          int content_height =
              SCREEN_HEIGHT - BLE_CONTENT_Y_START - BLE_CONTENT_BOTTOM_MARGIN;
          s_ble_state.last_page_consumed = measure_wrapped_consumed(
              content_width, content_height, page_buffer, 0);
          draw_wrapped_text(BLE_CONTENT_X_MARGIN, BLE_CONTENT_Y_START,
                            content_width, content_height, page_buffer, 0);

          // 调试日志：显示当前页读取情况
          ESP_LOGI(TAG,
                   "Page rendered: pos=%zu, read=%d, consumed=%zu, total=%zu",
                   s_ble_state.char_position, bytes_read,
                   s_ble_state.last_page_consumed, s_ble_state.total_chars);
        } else {
          // 读取返回0字节，说明已到文件末尾
          s_ble_state.last_page_consumed = 0; // 标记消耗为0，触发末尾检测
          display_draw_text_menu(20, 100, "已到章节末尾", COLOR_BLACK,
                                 COLOR_WHITE);
        ...
        display_draw_text_menu(20, 100, "正在加载...", COLOR_BLACK,
                               COLOR_WHITE);
      }
...
static void send_page_sync_notification(uint16_t page_num) {
  ...
  ESP_LOGI(TAG, "Sending chapter request: %s (book=0x%08lX, chapter=%d)", msg,
           (unsigned long)s_ble_state.current_book_hash, current_chapter);

  // Send via BLE
  bool sent = ble_manager_send_notification((const uint8_t *)msg, strlen(msg));
  ESP_LOGI(TAG, "Page notification send result: %s",
           sent ? "SUCCESS" : "FAILED");
}
...
static void send_page_request(bool is_forward) {
  ...
  ESP_LOGI(TAG, "[PAGE_REQUEST] %s - %s", is_forward ? "NEXT" : "PREV",
           sent ? "SENT" : "FAILED");
}
...
static void update_cached_window(uint16_t current_page) {
  ...
  ESP_LOGI(TAG, "VFS page window -> prev:%u curr:%u next:%u",
           s_ble_state.cached_pages[0], s_ble_state.cached_pages[1],
...
static inline void page_previous(void) {
  ...
  if (s_ble_state.current_page > 0) {
    ...
    if (s_ble_state.history_len > 0) {
      s_ble_state.char_position =
          s_ble_state.history_char_pos[--s_ble_state.history_len];
    } else {
      s_ble_state.char_position = 0;
    }
    ESP_LOGI(TAG, "Page previous: %u (seek=%zu)", s_ble_state.current_page,
             s_ble_state.char_position);
    ...
    if (s_ble_state.device_connected) {
      // 蓝牙已连接 → 请求上一章
      ESP_LOGI(TAG,
               "Page previous: at first page, requesting previous chapter");
      send_page_request(false); // 向后翻（上一章）
    } else {
      // 蓝牙未连接 → 显示提示
      ESP_LOGW(TAG, "Page previous: at first page, but BLE disconnected");
      display_draw_text_menu(20, SCREEN_HEIGHT / 2, "已到章节开头", COLOR_BLACK,
                             COLOR_WHITE);
      display_draw_text_menu(20, SCREEN_HEIGHT / 2 + 30, "请连接蓝牙加载上一章",
                             COLOR_BLACK, COLOR_WHITE);
      display_refresh(REFRESH_MODE_PARTIAL);
    }
  }
...
/** @brief 翻到下一页：保存当前页到历史，计算新的 seek
 * 位置，到达末尾时请求下一章 */
static inline void page_next(void) {
  ...
  bool can_next = (s_ble_state.total_pages == 0 ||
  ...
  if (can_next && !at_content_end) {
    ...
    // 保存当前页起始到历史栈
    if (s_ble_state.history_len <
        (uint8_t)(sizeof(s_ble_state.history_char_pos) /
                  sizeof(s_ble_state.history_char_pos[0]))) {
      s_ble_state.history_char_pos[s_ble_state.history_len++] =
          s_ble_state.char_position;
    }
    ...
    s_ble_state.char_position += s_ble_state.last_page_consumed;
    s_ble_state.current_page++;
    ESP_LOGI(TAG, "   新位置: page=%u, seek=%zu", s_ble_state.current_page,
             s_ble_state.char_position);
    ...
    if (s_ble_state.device_connected) {
      ...
      // 显示加载提示
      display_clear(COLOR_WHITE);
      display_draw_text_menu(20, 100, "正在加载下一章...", COLOR_BLACK,
                             COLOR_WHITE);
      display_refresh(REFRESH_MODE_FULL);
      ...
      display_draw_text_menu(20, SCREEN_HEIGHT / 2, "已到章节末尾", COLOR_BLACK,
                             COLOR_WHITE);
      display_draw_text_menu(20, SCREEN_HEIGHT / 2 + 30, "请连接蓝牙加载下一章",
                             COLOR_BLACK, COLOR_WHITE);
      display_refresh(REFRESH_MODE_PARTIAL);
    }
  }
...
static inline void scroll_down(void) {
  ...
  if (step == 0) {
    ...
    ESP_LOGW(TAG, "Scroll DOWN: last_page_consumed=0, using estimate=%zu",
             step);
  }
  ...
  if (s_ble_state.total_chars > 0 && new_pos >= s_ble_state.total_chars) {
    // 已经到达文件末尾
    ESP_LOGI(TAG, "Scroll DOWN: at end (pos=%zu, total=%zu)", new_pos,
             s_ble_state.total_chars);
  ...
  } else if (s_ble_state.total_chars == 0 &&
             s_ble_state.last_page_consumed == 0) {
    // 文件为空或未加载
    ...
    if (s_ble_state.device_connected) {
      ...
      display_draw_text_menu(20, SCREEN_HEIGHT / 2, "内容为空", COLOR_BLACK,
                             COLOR_WHITE);
      display_draw_text_menu(20, SCREEN_HEIGHT / 2 + 30, "请连接蓝牙加载内容",
                             COLOR_BLACK, COLOR_WHITE);
      display_refresh(REFRESH_MODE_PARTIAL);
    }
  } else {
    ...
    ESP_LOGI(TAG, "Scroll DOWN: seek=%zu (consumed=%zu, in page)",
...
static void handle_reading_mode_button(screen_t *screen, button_t btn) {
  ...
  case BTN_CONFIRM:
    ...
    if (s_ble_state.pending_local_resume) {
      ...
      if (localf == NULL) {
        ESP_LOGE(TAG, "Failed to open resume file: %s",
                 s_ble_state.resume_path);
        display_draw_text_menu(20, 100, "未找到本地缓存文件", COLOR_BLACK,
                               COLOR_WHITE);
        display_refresh(REFRESH_MODE_PARTIAL);
      ...
      if (stat(s_ble_state.resume_path, &st) == 0) {
        ...
        ESP_LOGI(TAG, "Loaded local file size: %zu bytes",
                 s_ble_state.total_chars);
      } else {
    ...
    if (s_ble_state.current_book_id != 0 &&
      ...
      s_ble_state.initialization_complete = true;
      ESP_LOGI(TAG,
               "Book initialization confirmed, starting to send initial pages");
      ...
      ble_manager_send_notification((const uint8_t *)notify_msg,
                                    strlen(notify_msg));
...
static void on_show(screen_t *screen) {
  ...
  if (s_ble_state.total_chars > 0 && s_ble_state.vfs_book != NULL) {
    vfs_seek(s_ble_state.vfs_book, s_ble_state.char_position, SEEK_SET);
    ESP_LOGI(TAG, "Restored reading position: %zu/%zu",
             s_ble_state.char_position, s_ble_state.total_chars);
  }
  ...
  do {
    ...
    if (stat(resume_path, &st) == 0 && st.st_size > 0) {
      ...
      s_ble_state.pending_local_resume = true;
      snprintf(s_ble_state.resume_path, sizeof(s_ble_state.resume_path), "%s",
               resume_path);
      ...
      ESP_LOGI(TAG, "Local resume available: %s size=%zu pos=%zu",
               s_ble_state.resume_path, s_ble_state.total_chars,
               s_ble_state.char_position);
    }
  } while (0);
...
static void request_chapter_list(void) {
  ...
  char book_id[32];
  snprintf(book_id, sizeof(book_id), "0x%08lX",
           (unsigned long)s_ble_state.current_book_hash);
...
static void parse_chapter_info_json(const char *json_data, size_t json_len) {
  ...
  // 预期格式:
  // {"book_title":"三体","chapters":[{"index":0,"title":"第一章","page_count":45,"available":true},...]}
  ...
  while (*ptr && s_ble_state.chapter_list_count < 64) {
    ...
    if (index_str) {
      index_str += 8;
      s_ble_state.chapter_list[s_ble_state.chapter_list_count].index =
          atoi(index_str);
    }
      ...
      if (title_end) {
        ...
        if (len > 0 && len < 128) {
          memcpy(s_ble_state.chapter_list[s_ble_state.chapter_list_count].title,
                 title_str, len);
          s_ble_state.chapter_list[s_ble_state.chapter_list_count].title[len] =
              '\0';
        }
      }
    }
    ...
    if (pages_str) {
      pages_str += 13;
      s_ble_state.chapter_list[s_ble_state.chapter_list_count].page_count =
          atoi(pages_str);
    }
  ...
  ESP_LOGI(TAG, "📖 Parsed %d chapters from JSON",
           s_ble_state.chapter_list_count);
...
static void __attribute__((unused)) enter_chapter_browser(void) {
  ...
  s_ble_state.chapter_list_selection =
      vfs_get_current_chapter(s_ble_state.vfs_book);
  ...
  if (s_ble_state.chapter_list_count == 0) {
    ...
    // 显示加载提示
    display_clear(0xFF);
    display_draw_text_font(SCREEN_WIDTH / 2 - 100, 60, "正在获取章节列表...",
                           &Font20, 0, 0xFF);
    display_refresh(REFRESH_MODE_FULL);
  } else {
    // 直接显示章节列表
...
static void draw_chapter_browser_screen(void) {
  ...
  if (s_ble_state.chapter_list_count == 0) {
    // 无章节数据
    display_draw_text_font(SCREEN_WIDTH / 2 - 60, 100, "无可用章节", &Font20, 0,
                           0xFF);
    display_draw_text_font(SCREEN_WIDTH / 2 - 70, 130, "按任意键返回", &Font16,
                           0, 0xFF);
    return;
  }
  ...
  for (int i = start_idx; i < end_idx; i++) {
    ...
    // 章节标题（截断长标题）
    snprintf(line_buf, sizeof(line_buf), "%s %d. %.40s", marker, i + 1,
             s_ble_state.chapter_list[i].title);
  ...
  // 底部提示
  display_draw_text_font(10, SCREEN_HEIGHT - 20, "Vol+/-选择  OK选中  Back返回",
                         &Font12, 0, 0xFF);
}
...
static void handle_chapter_browser_button(screen_t *screen, button_t btn) {
    ...
    if (s_ble_state.chapter_list_selection > 0) {
      ...
      do {
        s_ble_state.chapter_list_selection--;
               ...
               !s_ble_state.chapter_list[s_ble_state.chapter_list_selection]
                    .available);
  ...
  case BTN_VOLUME_DOWN: // 下一章
    if (s_ble_state.chapter_list_selection <
        s_ble_state.chapter_list_count - 1) {
      // 跳过不可用的章节
      do {
        s_ble_state.chapter_list_selection++;
      } while (s_ble_state.chapter_list_selection <
                   s_ble_state.chapter_list_count &&
               !s_ble_state.chapter_list[s_ble_state.chapter_list_selection]
                    .available);
 ...
 */
static void select_and_load_chapter(int chapter_index) {
  ESP_LOGI(TAG, "📖 User selected chapter %d: %s", chapter_index + 1,
           s_ble_state.chapter_list[chapter_index].title);

  // 发送 SELECT_CHAPTER 命令到 Client
  uint8_t cmd[4] = {0xA5, 0x5A,              // Magic
  ...
  if (ble_reader_switch_chapter(chapter_index)) {
    ...
    // 显示加载提示
    display_clear(0xFF);
    display_draw_text_font(SCREEN_WIDTH / 2 - 80, 100, "正在加载章节...",
                           &Font20, 0, 0xFF);
    display_refresh(REFRESH_MODE_FULL);
    ...
    // 显示错误提示
    display_clear(0xFF);
    display_draw_text_font(SCREEN_WIDTH / 2 - 70, 100, "章节加载失败", &Font20,
                           0, 0xFF);
    display_draw_text_font(SCREEN_WIDTH / 2 - 70, 130, "按任意键返回", &Font16,
                           0, 0xFF);
    display_refresh(REFRESH_MODE_FULL);
